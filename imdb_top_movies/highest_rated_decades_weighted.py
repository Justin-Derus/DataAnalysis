...
imdb_table = pd.read_csv('./imdb_top_movies.csv', index_col = 0)             # DataFrame created from .csv file
...
# Goal of chunk - weigh the decades by the number of movies they have
counts = decade_highest_rated.groupby('Decade',as_index=False)['Rating'].count()         # Find the count of movies  
...
cbar = plt.colorbar(sm,orientation='horizontal')       # Create the colorbar using our custom ScalarMappable object
cbar.set_label("Numer of Movies in Decade")            # Give colorbar a label
...
# Visual zooming
plt.ylim(min_rating - .25, max_rating + .1)  # zoom in based on min and max ratings, without this graph is huge and all look the same height

# Titles and axes labels
plt.title("Highest Rated Decade (Top 250 IMBD Movies)\n(Weighted by Number of Movies)")
plt.xlabel("Decade")
...
# Y = mx + b, m is the coefficient (slop), b is the y-intercept/constant (tells you Y when X is 0)
coeff_weighted = np.polyfit(avg_rating_per_decade['Decade'],   # X-Coord
                          avg_rating_per_decade['Rating'],     # Y-Coord
                          1,                                   # 1 = Linear (answers overall), 2 = Quadratic (overall but slight curvature)
...
for bar in bars:                                 # in order to get text, we need to go through all bar objects (matplotlib.patches.Rectangle)
    plt.text(bar.get_x() + bar.get_width() / 2,  # X-coord of left edge of bar, get_width / 2 gives half width which is distance from left edge to center of bar
             bar.get_height() + 0.02,            # Y-coord (Verticality)
             bar.get_height(),                   # What is displayed
             ha='center')                        # Center the text (horizontal alignment)

plt.legend(framealpha = 0.4) # Give opacity to the legend 
